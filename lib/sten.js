...
function SHA256_to_base(string, base) {
  if (typeof string === "string" && typeof base === "number" && base > 1) {
    ...
    //res = eval(res)
    let res = CryptoJS.SHA256(string)
      .words.map((i) => (i = new Uint32Array([i])[0].toString(base)))
      .join("");
    return res;
  }
  return -1;
...
let bin_to_str = (bin_str, char_size) => {
  ...
  for (let i = 0; i < bin_str.length; i = i + char_size) {
    ...
    char_arr.push(
      String.fromCharCode(parseInt(bin_str.substring(i, i + char_size), 2))
    );
  }
...
let decode_consistant = (arr, start, size, bits) => {
  ...
  let res_str = "";
  if (distance(start, arr.length, bits) < size) return -1;
...
let insertflag = (arr, start, flag) => {
  ...
  for (buff_p; buff_p < flag.length; buff_p = buff_p + 2) {
    if (index >= arr.length * 4) return -1;
    let val;
    if (buff_p + 1 < flag.length)
      val = set_value(
        arr[index],
        parseInt(flag.substring(buff_p, buff_p + 2), 2),
        4
      );
    else val = set_value(arr[index], parseInt(flag.charAt(buff_p), 2), 2);
...
let encode = (arr, text = " ", id = 0, part = 0, hash_str = str) => {
  ...
  if (distance(0, arr.length, 2) < flag.length) return -1;
  ...
  insertFlag_res = insertflag(
    arr,
    index_after_flag,
    temp_id.toString(2).padStart(16, "0").substring(0, 16)
  );
  ...
  insertFlag_res = insertflag(
    arr,
    index_after_flag,
    parseInt(part).toString(2).padStart(8, "0").toString(0, 8)
  );
  ...
  for (let str_p = 0; str_p < bin_str.length; str_p = str_p + 2) {
    ...
    if (
      str_p % char_size == 0 &&
      distance(index, end_index, 1) < cell_counter(char_size * base, bits)
    ) {
    ...
    if (str_p + 1 < bin_str.length) {
      val = set_value(
        arr[index],
        parseInt(bin_str.substring(str_p, str_p + 2), 2),
        4
      );
...
let decode = (arr, hash_str = str) => {
  //checkFlag
  if (
    checkflag(arr, 0, flag) == -1 ||
    (arr.length / 4) * 3 <= cell_counter(2 * flag.length + 16 + 8, 2)
  )
    return -1;
